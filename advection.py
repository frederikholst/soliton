...
u_kink_init = np.zeros([x_size, t_size], dtype=float)
u_soliton_init=np.copy(u_kink_init)
...
u_kink_init[:, 0] = np.array([init(x) for x in x_series]) 
...
u_solution_sol = KdV_Solver(u_soliton_init, Delta_x, Delta_t) # Solutions for time up to T_max

u_solution_kink = KdV_Solver(u_kink_init, Delta_x, Delta_t, lin=True, mu=0)
...
u_kink_list=[u_solution_kink[:, int(t / Delta_t)] for t in T_series]
