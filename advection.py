...
t_0 = 10                                  # Initial time
u[:, 0] = np.array([init(x) for x in x_series])  # Soliton at t0

# Solve the KdV equation
u_solution = KdV_Solver(u, Delta_x, Delta_t, lin=True, mu=0) # Solutions for time up to T_max
...
# Save zip file:
np.savez('Data\\advection.npz', u_sol=u_sol_list, x_series=x_series)